...
async def get_current_user_controller(db: Session = Depends(get_db), 
                     current_user: schemas.User = Depends(get_current_user)):
    return current_user

@router.post(path='/forgot-password/',
             status_code=status.HTTP_200_OK)
async def get_password_reset_link(user: schemas.ForgotPassword,
                            request: Request,
                            db: Session = Depends(get_db)):
    return await crud.get_password_reset_link(user=user, host_url=request.base_url, db=db)
