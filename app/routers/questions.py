...
def get_all_questions(answered: bool = None, page: int = 1, limit: int = 10, db: Session = Depends(get_db)):
    return crud.get_all_questions(answered=answered, page=page, limit=limit, db=db)
...
def delete_question(id: int, db: Session = Depends(get_db), current_admin = Depends(get_current_admin)):
