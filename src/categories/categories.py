...
def create_category(data: schemas.CategoryCreate, db: Session = Depends(get_db), current_user: users.schemas.User = Depends(auth.utils.get_current_admin)):
...
def update_category(id: int, data: schemas.CategoryUpdate, db: Session = Depends(get_db), current_user: users.schemas.User = Depends(auth.utils.get_current_admin)):
...
def delete_category(id: int, db: Session = Depends(get_db), current_user: users.schemas.User = Depends(auth.utils.get_current_admin)):
    query = db.query(models.Category).filter(models.Category.id == id)
    if(not query.first()):
        raise exceptions.ResourceNotFound
    query.delete()
